# -*- coding: utf-8 -*-
"""
BongDaPlus Parser - Parser cho Bóng Đá Plus
"""
...
class BongDaPlusParser(BaseParser):
    """Parser cho Bóng Đá Plus"""
    
    def __init__(self):
        super().__init__('BongDaPlus', 'https://bongdaplus.vn')
    ...
    def get_article_list(self, limit=10):
        ...
        # Tìm các bài viết (cần điều chỉnh selector theo cấu trúc web)
        article_items = soup.select('.article-item, .news-item')[:limit]
        ...
        for item in article_items:
            try:
                ...
                # Lấy mô tả
                desc_tag = item.select_one('.description, .sapo')
        ...
        logger.info(f"✓ Tìm thấy {len(articles)} bài viết từ BongDaPlus")
        return articles
    ...
    def parse_article(self, url):
        ...
        try:
            ...
            # Lấy tiêu đề
            title_tag = soup.select_one('h1.article-title, h1.title-detail')
            if not title_tag:
            ...
            # Lấy mô tả
            desc_tag = soup.select_one('.article-sapo, .sapo')
            summary = self.clean_text(desc_tag.get_text()) if desc_tag else ''
            
            # Lấy nội dung
            content_tag = soup.select_one('.article-content, .detail-content')
            if not content_tag:
            ...
            # Lấy thumbnail
            thumb_tag = soup.select_one('.article-thumbnail img, .detail-thumb img')
            thumbnail_url = ''
            if thumb_tag:
